...
// disable some warnings for this file
#ifdef TYCHO_PC
#pragma warning(disable : 4210 4100 4113 4054 4131 4389 4055 4018 4244 4702 4701)
#endif // TYCHO_PC
...
struct lemon {
  ...
  char *tpltfilename;      /* Name of template file */
#endif  
...
#define PREFIXLIMIT    100 /* Max width of the prefix on each line */
...
#ifdef TYCHO  
    sprintf(prefix,"%.*s(0): ",PREFIXLIMIT-10,filename);
#else    
...
char **argv)
{
  ...
  if( version ){
     printf("Lemon version 1.0 (Tycho 2008 Remix)\n");
     exit(0); 
  }
  if( OptNArgs()!=3 ){
    fprintf(stderr,"Exactly three filename arguments are required.\n");
    fprintf(stderr,"Usage : lemon.exe <input_file> <output_file> <template_file>\n");
    exit(1);
  }
...
#ifdef TYCHO  
  lem.outfilename = OptArg(1);
  lem.tpltfilename = OptArg(2);
#endif
...
#ifdef TYCHO
  name = malloc( strlen(lemp->outfilename) + strlen(suffix) + 5 );
  if( name==0 ){
    fprintf(stderr,"Can't allocate space for a filename.\n");
    exit(1);
  }
  strcpy(name,lemp->outfilename);
#else
...
char *mode;
{
  ...
  {
	fprintf(fp, "///////////////////////////////////////////////////////////////////////\n");
	fprintf(fp, "// GENERATED BY LEMON.EXE DO NOT MODIFY!!\n");
	fprintf(fp, "///////////////////////////////////////////////////////////////////////\n");
  }
#endif  
  if( fp==0 && *mode=='w' ){
...
#ifdef TYCHO  
  if( access(lemp->tpltfilename, 004)==0 ){
	tpltname = lemp->tpltfilename;
  } else 
#endif  
  if( access(buf,004)==0 ){
    tpltname = buf;
